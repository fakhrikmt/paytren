...
var browserApi = function() {
  "use strict";
  var isInject = location.protocol !== 'moz-extension:';
  var isBgPage = false;
  !isInject && (function () {
    isBgPage = location.pathname.indexOf('_generated_background_page.html') !== -1;
    if (!isBgPage) {
      // ff < 48
      isBgPage = location.pathname.indexOf('_blank.html') !== -1;
    }
  ...
  var msgTools = {
    ...
    listener: function(message, sender, sendResponse) {
      var _this = msgTools;
      if (message && message.mono && message.idPrefix !== _this.idPrefix && message.isBgPage !== isBgPage) {
        ...
        if (message.hasCallback) {
          hasCallback = true;
          responseFn = onceFn(function(message) {
            hasCallback = false;
        ...
        _this.listenerList.forEach(function(fn) {
          if (message.hook === fn.hook) {
            var r = fn(message.data, responseFn);
            if (r === true) {
              result = true;
            }
          }
  ...
  (function (api) {
    ...
    api.executeScripts = function (tab, details) {
      for (var i = 0, file; file = details.files[i]; i++) {
        browser.tabs.executeScript(tab.id, {file: file, runAt: 'document_end'});
      }
    };
    /**
     * @param {Function} callback
  ...
  mono.getDataUrl = function (content, type, isForceData) {
    ...
    if (!isForceData && typeof URL !== 'undefined' && typeof Blob !== "undefined" && !mono.isSafari) {
      var blob = new Blob([content], {encoding: "UTF-8", type: type});
